...
void find_key_length(const char *ciphertext, int max_key_len); // NEW
...
int main() {
    ...
    // ----------------------------------------------------
    printf("\n--- Phase 2: Key Length Determination ---\n");
    find_key_length(cleaned_ciphertext, 15); 
    
    // *** MANUAL STEP: ASSUME BEST KEY LENGTH ***
    int assumed_key_len = 5; 
    printf("\n*** Assuming most probable Key Length (L) is %d for key recovery. ***\n", assumed_key_len);

    // ----------------------------------------------------
    // --- Phase 3: Key Recovery ---
    // ----------------------------------------------------
    char key_buffer[assumed_key_len + 1];
    printf("\n--- Phase 3: Key Recovery ---\n");
    
    if (find_key(cleaned_ciphertext, assumed_key_len, key_buffer)) {
 ...
 */
void find_key_length(const char *ciphertext, int max_key_len) {
    ...
    for (int L = 1; L <= max_key_len; L++) {
        ...
        for (int i = 0; i < L; i++) {
            ...
            size_t sub_text_max_len = (N / L) + 1; 
            
            // Dynamic allocation is safer for large texts, but we'll use a VLA (if supported) 
            // or a small, fixed array for simplicity, as sub-text size is predictable.
            // For production, use malloc/realloc. Here, let's simplify with a temporary small array.
            
            // NOTE: A VLA (Variable Length Array) is used here for simplicity. 
            // If your compiler doesn't support VLA, you must use malloc/free for the sub_text_buffer.
            char sub_text_buffer[sub_text_max_len];
            int sub_text_idx = 0;
...
char find_key_letter(const char *sub_text) {
    ...
    if (N < 5) return '?'; // Too short for reliable analysis
...
int find_key(const char *ciphertext, int key_len, char *key_buffer) {
    size_t N = strlen(ciphertext);
    if (key_len <= 0 || key_len > N) return 0;
